...
const DEFAULT_CONFIG = {
    ...
    DRAG_SORT_DELAY: 500,
    ENGINE_BAR_OFFSET_DEFAULT: 0,  // 这里缺少逗号
    INPUT_METHOD_CHECK_INTERVAL: 100, 
    KEYBOARD_CLOSE_DETECTION_DELAY: 300,
    KEYBOARD_CLOSE_CLICK_DELAY: 200, // 点击检测键盘收起的延迟
    FORCE_BLUR_CHECK_INTERVAL: 1000  // 强制失焦检查间隔
};
...
const appState = {
    ...
    // 新增：标记是否正在与引擎按钮栏交互
    isInteractingWithEngineBar: false,  // 这里缺少逗号
    inputMethodCheckInterval: null,
    keyboardVisible: false,
    lastKeyboardShowTime: 0,
    forceBlurCheckInterval: null,
    isManuallyCheckingBlur: false
};
...
const utils = {
    ...
    detectInputMethodState() {
        ...
        // 方法1: 通过视口高度变化检测（移动端有效）
        const viewportHeight = window.visualViewport ? window.visualViewport.height : window.innerHeight;
        const windowHeight = window.innerHeight;
        
        // 如果可视视口高度明显小于窗口高度，说明键盘弹出
        const keyboardThreshold = 300;
        const isKeyboardByViewport = (windowHeight - viewportHeight) > keyboardThreshold;
        
        // 方法2: 通过输入框位置检测
        let isKeyboardByPosition = false;
        if (activeElement) {
            const rect = activeElement.getBoundingClientRect();
            const inputBottom = rect.bottom;
            const viewportHeight = window.visualViewport ? window.visualViewport.height : window.innerHeight;
            
            isKeyboardByPosition = inputBottom > (viewportHeight * 0.7);
        }
        
        // 方法3: 移动端特定检测 - 检查页面滚动状态
        let isKeyboardByScroll = false;
        if (isInputFocused && !isKeyboardByViewport) {
            // 如果输入框聚焦但视口没变化，可能是键盘已弹出但浏览器没正确报告
            // 检查页面是否被键盘推上去
            const bodyHeight = document.body.clientHeight;
            const windowInnerHeight = window.innerHeight;
            isKeyboardByScroll = (bodyHeight > windowInnerHeight * 1.5);
        }
        
        // 综合判断键盘状态
        const wasKeyboardVisible = appState.keyboardVisible;
        appState.keyboardVisible = isKeyboardByViewport || isKeyboardByPosition || isKeyboardByScroll;
        
        // 记录键盘显示时间
        if (appState.keyboardVisible && !wasKeyboardVisible) {
            appState.lastKeyboardShowTime = Date.now();
        }
        
        // 如果键盘状态发生变化，更新UI
        if (wasKeyboardVisible !== appState.keyboardVisible) {
            domHandler.updateSearchBoxPosition();
        }
        
        return appState.keyboardVisible;
    },
    ...
    forceCheckBlurState() {
        ...
        // 如果没有输入框聚焦，但键盘状态还是显示，强制更新
        if (!isInputFocused && appState.keyboardVisible) {
            appState.keyboardVisible = false;
            domHandler.updateSearchBoxPosition();
        }
    },
     ...
     */
    startForceBlurCheck() {
        if (appState.forceBlurCheckInterval) {
            clearInterval(appState.forceBlurCheckInterval);
        }
        
        appState.forceBlurCheckInterval = setInterval(() => {
            this.forceCheckBlurState();
        }, DEFAULT_CONFIG.FORCE_BLUR_CHECK_INTERVAL);
    },
    ...
    stopForceBlurCheck() {
        if (appState.forceBlurCheckInterval) {
            clearInterval(appState.forceBlurCheckInterval);
            appState.forceBlurCheckInterval = null;
        }
    },
     ...
     * 手动触发键盘收起检测
     */
    manualKeyboardHideCheck() {
        // 避免频繁触发
        if (appState.isManuallyCheckingBlur) return;
        
        appState.isManuallyCheckingBlur = true;
        
        setTimeout(() => {
            this.detectInputMethodState();
            this.forceCheckBlurState();
            appState.isManuallyCheckingBlur = false;
        }, DEFAULT_CONFIG.KEYBOARD_CLOSE_CLICK_DELAY);
    }
};
    ...
    stopInputMethodMonitoring() {
        if (appState.inputMethodCheckInterval) {
            clearInterval(appState.inputMethodCheckInterval);
            appState.inputMethodCheckInterval = null;
        }
    },
...
const domHandler = {
   ...
   /**
    ...
    monitorInputFields() {
        const setupInputMonitoring = (input) => {
            ...
            input.addEventListener('focus', (e) => {
                utils.startInputMethodMonitoring();
                utils.startForceBlurCheck(); // 新增：开始强制失焦检查
                
                setTimeout(() => {
                    utils.detectInputMethodState();
                    domHandler.updateSearchBoxPosition();
                }, 100);
            });

            input.addEventListener('blur', (e) => {
                utils.stopInputMethodMonitoring();
                utils.stopForceBlurCheck(); // 新增：停止强制失焦检查
                ...
                }, DEFAULT_CONFIG.KEYBOARD_CLOSE_DETECTION_DELAY);
            });
     ...
     * 初始化页面点击监听（检测键盘收起）
     */
    initPageClickForKeyboardHide() {
        // 监听整个文档的点击事件
        document.addEventListener('click', (e) => {
            // 如果当前有键盘显示，且点击的不是输入框
            if (appState.keyboardVisible) {
                const isClickOnInput = e.target.matches('input, textarea') || 
                                      e.target.closest('input, textarea');
                
                if (!isClickOnInput) {
                    // 可能是点击了键盘收起按钮或其他地方，触发键盘收起检测
                    utils.manualKeyboardHideCheck();
                }
            }
        }, true); // 使用捕获阶段确保能捕获到所有点击
        
        // 监听触摸开始事件（移动端）
        document.addEventListener('touchstart', (e) => {
            if (appState.keyboardVisible) {
                const isTouchOnInput = e.target.matches('input, textarea') || 
                                      e.target.closest('input, textarea');
     ...
     * 初始化移动端特定的事件监听
     */
    initMobileSpecificListeners() {
        // 监听页面可见性变化
        document.addEventListener('visibilitychange', () => {
            if (document.hidden && appState.keyboardVisible) {
                // 页面隐藏时，假设键盘已收起
                appState.keyboardVisible = false;
                domHandler.updateSearchBoxPosition();
            }
    ...
    updateSearchBoxPosition() {
        ...
        ) && (appState.keyboardVisible || appState.isInteractingWithEngineBar === false);
 ...
 /**
    ...
    init() {
        try {
            ...
            domHandler.initViewportListener();
            domHandler.initPageClickForKeyboardHide();    // 新增
            domHandler.initPageScrollForKeyboardHide();   // 新增
            domHandler.initMobileSpecificListeners();     // 新增
            accessibility.init();

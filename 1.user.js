...
// 扩展存储键
STORAGE_KEYS.ENGINE_BAR_OFFSET = 'engineBarOffset';

// 扩展默认配置
DEFAULT_CONFIG.ENGINE_BAR_OFFSET_DEFAULT = 0;

// 修改 updateSearchBoxPosition 支持输入法偏移检测
const originalUpdateSearchBoxPosition = domHandler.updateSearchBoxPosition;
domHandler.updateSearchBoxPosition = function() {
    const punkJetBox = document.getElementById("punkjet-search-box");
    if (!punkJetBox) return;

    const offsetValue = GM_getValue(STORAGE_KEYS.ENGINE_BAR_OFFSET, DEFAULT_CONFIG.ENGINE_BAR_OFFSET_DEFAULT);
    const activeElement = document.activeElement;
    const shouldOffset = activeElement && (
...
// 添加输入框焦点和输入法检测监听
window.addEventListener('resize', () => domHandler.updateSearchBoxPosition());
document.addEventListener('focusin', () => domHandler.updateSearchBoxPosition());
document.addEventListener('focusout', () => domHandler.updateSearchBoxPosition());

// ===== 汉堡菜单增强：增加设置底部偏移按钮 =====
const originalShowHamburgerMenu = domHandler.showHamburgerMenu;
domHandler.showHamburgerMenu = function() {
    originalShowHamburgerMenu.call(this);

    const menu = document.getElementById(CLASS_NAMES.HAMBURGER_MENU);
    if (!menu) return;

    if (!menu.querySelector('#set-offset-btn')) {
        const setOffsetButton = document.createElement('button');
        setOffsetButton.id = 'set-offset-btn';
        setOffsetButton.innerHTML = utils.createInlineSVG('cog') + ' 设置底部偏移';
        setOffsetButton.style.cssText = 'margin-top:10px;width:100%;height:32px;cursor:pointer;border:none;border-radius:8px;background:#f8f8f8;';
        setOffsetButton.addEventListener('click', () => {
            const currentValue = GM_getValue(STORAGE_KEYS.ENGINE_BAR_OFFSET, DEFAULT_CONFIG.ENGINE_BAR_OFFSET_DEFAULT);
            const userValue = prompt(`请输入搜索栏在输入法弹出时的底部偏移（单位px）：`, currentValue);
            if (userValue !== null && !isNaN(userValue)) {
                GM_setValue(STORAGE_KEYS.ENGINE_BAR_OFFSET, parseInt(userValue));

...
`
    ...
    `
         ...
         // ===== 常量定义区 =====
            ...
            // 存储键名常量（统一管理GM存储键）
            const STORAGE_KEYS = Object.freeze({USER_SEARCH_ENGINES: 'userSearchEngines',
    ...
    , ENGINE_BAR_OFFSET: 'engineBarOffset'
            ...
            // 默认配置（抽离默认值，便于维护）
            const DEFAULT_CONFIG = {PUNK_DEFAULT_MARK: 'Bing-Google-Baidu-MetaSo-YandexSearch-Bilibili-ApkPure-Quark-Zhihu',
    ...
    , ENGINE_BAR_OFFSET_DEFAULT: 0
        ...
        `;
                ...
                updateSearchBoxPosition() {
                    ...
                    // 基础位置配置（保留原有宽度/left设置）
                    punkJetBox.style.left = "2%";
                    punkJetBox.style.width = "96%";

                    // 读取用户设置的固定偏移值（px）
                    const offsetValue = GM_getValue(STORAGE_KEYS.ENGINE_BAR_OFFSET, DEFAULT_CONFIG.ENGINE_BAR_OFFSET_DEFAULT) || 0;

                    // 判断是否需要应用偏移：当页面中有聚焦的输入框或textarea时应用
                    const active = document.activeElement;
                    const isInputFocused = active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA' || active.isContentEditable);

                    // 如果脚本状态为隐藏，则仍然保留隐藏样式（translateY）
                    if (appState.punkJetBoxVisible) {
                        // 如果输入框聚焦，则将底部设置为偏移值，否则为0
                        punkJetBox.style.bottom = isInputFocused ? `${offsetValue}px` : '0px';
        ...
        `;
            ...
            `;
                        ...
                        // 额外：监听输入焦点/失焦与窗口大小变化，用于在输入法唤醒时应用用户设置的固定偏移
                        const applyOffsetListeners = () => {
                            window.addEventListener('resize', () => this.updateSearchBoxPosition(), {passive:true});
                            document.addEventListener('focusin', (e) => this.updateSearchBoxPosition());
                            document.addEventListener('focusout', (e) => {
                                // 小延迟，确保焦点切换后的状态被正确读取（避免立刻恢复导致闪烁）
                                setTimeout(() => this.updateSearchBoxPosition(), 60);
                            });
                        };
                        applyOffsetListeners();
                    ...
                    const menuItems = [{
                        ...
                        }
                        {
                            ...
                            action: () => {
                                ...
                                if (userValue !== null) {
                                    const v = parseInt(userValue);
                                    if (!isNaN(v)) {
                                        GM_setValue(STORAGE_KEYS.ENGINE_BAR_OFFSET, v);
                                        alert('偏移值已设置为 ' + v + 'px');
                                        // 立即应用
                                        const menu = document.getElementById(CLASS_NAMES.HAMBURGER_MENU);
                                        if (menu) menu.style.display = 'none';
                                        // 更新位置
                                        const db = document.querySelector('.' + CLASS_NAMES.ENGINE_CONTAINER);
                                        if (db) domHandler.updateSearchBoxPosition();
                                    } else {
                                        alert('请输入有效的数字（单位px）');
                                    }
                                }

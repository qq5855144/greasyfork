...
const appState = {
    ...
    initialViewportHeight: window.innerHeight  // 添加这一行
};

// ===== 移动端适配模块 =====
/**
 * 移动端适配模块 - 处理输入法键盘、横竖屏切换等移动端特性
 */
const mobileAdapter = {
    /**
     * 初始化移动端适配
    ...
    init() {
        ...
        if (appState.isMobile) {
            this.initMultipleDetectionMethods();
     ...
     * 初始化多种检测方法
     */
    initMultipleDetectionMethods() {
        // 方法1: visualViewport API (现代浏览器)
        ...
        // 方法3: 输入框焦点事件检测
        this.initInputFocusListener();
        
        // 方法4: 定时器轮询检测
        this.initPollingDetection();
        
        // 方法5: 基于视口高度的检测
        this.initViewportHeightDetection();
    },

    /**
     * 方法1: 使用 visualViewport API
     */
    initViewportListener() {
        if (window.visualViewport) {
            ...
            console.log("使用 visualViewport API 检测键盘");
        } else {
            console.log("visualViewport API 不支持，使用备用方案");
        }
    },
        ...
        window.addEventListener('resize', () => {
            ...
            resizeTimeout = setTimeout(() => {
                ...
                if (Math.abs(heightDiff) > 100) {
                    this.handleWindowResize(heightDiff);
                }
     ...
     * 方法3: 输入框焦点事件检测
     */
    initInputFocusListener() {
        // 监听所有输入框的焦点事件
        document.addEventListener('focusin', (e) => {
            if (e.target.matches('input, textarea, [contenteditable="true"]')) {
                // 延迟检测，确保键盘已经弹出
                setTimeout(() => {
                    this.detectKeyboardByViewport();
                }, 300);
            }
        });
    ...
    initPollingDetection() {
        ...
        setInterval(() => {
            ...
            // 如果高度变化显著，认为是键盘状态变化
            if (Math.abs(currentHeight - lastViewportHeight) > 200) {
                this.handleViewportChange(currentHeight, lastViewportHeight);
     ...
     * 方法5: 基于视口高度的检测
     */
    initViewportHeightDetection() {
        // 存储初始视口高度
        if (!appState.initialViewportHeight) {
            appState.initialViewportHeight = window.innerHeight;
        }
    ...
    detectKeyboardByViewport() {
        const currentHeight = window.innerHeight;
        const initialHeight = appState.initialViewportHeight || window.screen.availHeight;
     ...
     * 处理视口变化
     */
    handleViewportChange(currentHeight, lastHeight) {
     ...
     * 处理窗口大小变化
     */
    handleWindowResize(heightDiff) {
        const KEYBOARD_THRESHOLD = 1000;
    ...
    /**
     * 根据键盘高度调整搜索框位置
     * @param {number} keyboardHeight 键盘高度
     */
    adjustForKeyboard(keyboardHeight) {
        const searchContainer = document.querySelector(`.${CLASS_NAMES.ENGINE_CONTAINER}`);
        if (!searchContainer) return;

        // 键盘高度阈值（通常键盘高度 > 100px）
        const KEYBOARD_THRESHOLD = 1000;
        const keyboardOffset = GM_getValue(STORAGE_KEYS.KEYBOARD_OFFSET, DEFAULT_CONFIG.KEYBOARD_OFFSET);
        
        if (keyboardHeight > KEYBOARD_THRESHOLD) {
            // 键盘弹出时，将搜索框容器移到键盘上方
            const newBottom = keyboardHeight + keyboardOffset;
            
            searchContainer.style.transition = 'bottom 0.3s ease';
            searchContainer.style.bottom = `${newBottom}px`;
            
            // 更新状态
            appState.keyboardVisible = true;
            appState.keyboardHeight = keyboardHeight;
            
            console.log(`键盘弹出，调整搜索框位置: bottom = ${newBottom}px`);
        } else {
            // 键盘收起时，恢复到底部
            this.resetSearchBoxPosition();
        }
    ...
    resetSearchBoxPosition() {
        ...
        // 只有在键盘可见时才重置
        if (appState.keyboardVisible) {
            searchContainer.style.transition = 'bottom 0.3s ease';
            searchContainer.style.bottom = '0px';
            
            // 更新状态
            appState.keyboardVisible = false;
            appState.keyboardHeight = 0;
            
            console.log("键盘收起，重置搜索框位置");
        }
    },
     ...
     */
    resetSearchBoxPosition() {
        const searchBox = document.querySelector(`.${CLASS_NAMES.ENGINE_CONTAINER}`);
        if (!searchBox) return;

        // 只有在键盘可见时才重置
        if (appState.keyboardVisible) {
            searchBox.style.transition = 'bottom 0.3s ease';
            searchBox.style.bottom = '0px';
    ...
    keepInViewport() {
        ...
        // 如果搜索框被键盘遮挡，调整位置
        if (searchBoxRect.bottom > viewportBottom) {
            const offset = searchBoxRect.bottom - viewportBottom + 10;
            const currentBottom = parseInt(searchBox.style.bottom || 0);
            searchBox.style.bottom = `${currentBottom + offset}px`;
        }
    },
    ...
    initOrientationChange() {
        window.addEventListener('orientationchange', () => {
            ...
            setTimeout(() => {
                appState.initialViewportHeight = window.innerHeight;
                this.forceDetectKeyboardState();
            }, 500);
        });

        // 备用方案：监听 resize 事件检测方向变化
        ...
        window.addEventListener('resize', () => {
            ...
            // 如果宽高比例发生显著变化，认为是横竖屏切换
            if (Math.abs(currentWidth - lastHeight) < 10 && Math.abs(currentHeight - lastWidth) < 10) {
                console.log('检测到横竖屏切换');
                setTimeout(() => {
                    appState.initialViewportHeight = window.innerHeight;
                    this.resetSearchBoxPosition();
                }, 300);
    ...
    getKeyboardStatus() {
        return {
            visible: appState.keyboardVisible,
            height: appState.keyboardHeight,
            isMobile: appState.isMobile,
            viewportHeight: window.innerHeight,
            initialViewportHeight: appState.initialViewportHeight
        };
    },
     ...
     */
    manualDetection() {
        console.log("手动触发键盘检测");
        this.forceDetectKeyboardState();
        
        const status = this.getKeyboardStatus();
...
const utils = {
    ...
    createInlineSVG(iconName, color = 'currentColor') {
        const icons = {
            ...
            keyboard: `<svg width="16" height="16" viewBox="0 0 16 16" fill="${color}">
                <path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h12zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2z"/>
                <path d="M3 5a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V5zm1 3a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V8zm1 3a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v1a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1v-1z"/>
            </svg>`
        };
...
const domHandler = {
    ...
    updateSearchBoxPosition() {
        ...
        // 基础位置配置 - 直接控制容器底部距离
        searchContainer.style.bottom = "0px";
        searchContainer.style.left = "2%";
        searchContainer.style.width = "96%";

        // 显示/隐藏状态切换
        searchContainer.style.transform = appState.punkJetBoxVisible ?
            "translateY(0)" :
            "translateY(100%)";
        searchContainer.style.opacity = appState.punkJetBoxVisible ? "1" : "0";
    },
...
const searchOverlay = {
     ...
     * 显示自定义设置面板
     */
    showCustomPanel(title, content, onReady) {
        const overlay = this.createSearchOverlay();
        const container = overlay.querySelector('div');
        
        // 替换内容为自定义面板
        container.innerHTML = `
            <div style="width: 90%; max-width: 400px; background: white; border-radius: 15px; padding: 20px;">
                <h2 style="margin: 0 0 15px 0; text-align: center;">${title}</h2>
                <div id="custom-panel-content">
                    ${content}
                </div>
            </div>
        `;
...
const hamburgerMenu = {
    ...
    showKeyboardSettings() {
        const keyboardStatus = mobileAdapter.getKeyboardStatus();
        const currentHeight = GM_getValue(STORAGE_KEYS.KEYBOARD_OFFSET, DEFAULT_CONFIG.KEYBOARD_OFFSET);
        
        const settingsHtml = `
            <div style="padding: 20px; max-width: 350px;">
                <h3 style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
                    ${utils.createInlineSVG('keyboard')} 键盘高度设置
                </h3>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="range" id="keyboard-offset" min="0" max="50" value="${currentHeight}" 
                           style="width: 100%; margin-bottom: 10px;">
                    <div style="display: flex; justify-content: space-between; font-size: 12px; color: #666;">
                        <span>0px</span>
                        <span id="offset-value">${currentHeight}px</span>
                        <span>50px</span>
                    </div>
                ...
                <div style="background: #f8f9fa; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
                    <div style="font-size: 12px; color: #666;">
                        <strong>检测状态:</strong><br>
                        <span id="detection-status">检测中...</span>
                    </div>
                    <div style="font-size: 12px; color: #666; margin-top: 5px;">
                        <strong>视口信息:</strong><br>
                        当前高度: <span id="current-height">${keyboardStatus.viewportHeight}px</span><br>
                        初始高度: <span id="initial-height">${keyboardStatus.initialViewportHeight}px</span><br>
                        键盘状态: <span id="keyboard-state">${keyboardStatus.visible ? '显示' : '隐藏'}</span>
                    </div>
                </div>
                
                <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                    <button id="test-detection" 
                            style="flex: 1; padding: 8px; background: #3498db; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 12px;">
                        测试检测
                    </button>
                    <button id="manual-adjust" 
                            style="flex: 1; padding: 8px; background: #9b59b6; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 12px;">
                        手动调整
                    </button>
                </div>
                
                <div style="display: flex; gap: 10px;">
                    <button id="save-keyboard-settings" 
                            style="flex: 1; padding: 10px; background: #27ae60; color: white; border: none; border-radius: 5px; cursor: pointer;">
                        保存
                    </button>
                    <button id="reset-keyboard-settings" 
        ...
        searchOverlay.showCustomPanel('键盘设置', settingsHtml, () => {
            const offsetInput = document.getElementById('keyboard-offset');
            ...
            if (offsetInput && offsetValue) {
                ...
                document.getElementById('test-detection').addEventListener('click', () => {
                    const status = mobileAdapter.manualDetection();
                    this.updateDetectionStatus(status);
                });
                
                // 手动调整按钮
                document.getElementById('manual-adjust').addEventListener('click', () => {
                    const offset = parseInt(offsetInput.value);
                    const searchContainer = document.querySelector(`.${CLASS_NAMES.ENGINE_CONTAINER}`);
                    if (searchContainer) {
                        searchContainer.style.bottom = `${offset}px`;
                        alert(`已手动调整搜索引擎栏到底部 ${offset}px 位置`);
                    }
                });
                
                // 保存按钮
                document.getElementById('save-keyboard-settings').addEventListener('click', () => {
                    const newOffset = parseInt(offsetInput.value);
                    GM_setValue(STORAGE_KEYS.KEYBOARD_OFFSET, newOffset);
                    searchOverlay.hideSearchOverlay();
                    alert('键盘高度设置已保存！');
                });
                
                // 重置按钮
                document.getElementById('reset-keyboard-settings').addEventListener('click', () => {
                    offsetInput.value = DEFAULT_CONFIG.KEYBOARD_OFFSET;
                    offsetValue.textContent = `${DEFAULT_CONFIG.KEYBOARD_OFFSET}px`;
                    GM_setValue(STORAGE_KEYS.KEYBOARD_OFFSET, DEFAULT_CONFIG.KEYBOARD_OFFSET);
    ...
    showKeyboardShortcuts() {
        const shortcutsHtml = `
            <div style="padding: 20px; max-width: 400px;">
                <h3 style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
                    ${utils.createInlineSVG('keyboard')} 快捷键说明
                </h3>
                
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                    <div style="display: grid; grid-template-columns: auto 1fr; gap: 10px; align-items: center;">
                        <kbd style="padding: 5px 10px; background: #2c3e50; color: white; border-radius: 4px; font-size: 12px;">Alt + S</kbd>
                        <span>打开快捷搜索</span>
                        
                        <kbd style="padding: 5px 10px; background: #2c3e50; color: white; border-radius: 4px; font-size: 12px;">Alt + M</kbd>
                        <span>打开/关闭菜单</span>
                        
                        <kbd style="padding: 5px 10px; background: #2c3e50; color: white; border-radius: 4px; font-size: 12px;">Alt + E</kbd>
                        <span>打开引擎管理</span>
                        
                        <kbd style="padding: 5px 10px; background: #2c3e50; color: white; border-radius: 4px; font-size: 12px;">ESC</kbd>
                        <span>关闭弹窗/菜单</span>
                    </div>
                </div>
                
                <div style="margin-top: 15px; font-size: 12px; color: #666;">
                    <p>💡 提示：在搜索框中输入关键词后，可以直接按回车使用默认搜索引擎搜索</p>
                </div>
                
                <button id="close-shortcuts" 
                        style="width: 100%; padding: 10px; background: #3498db; color: white; border: none; border-radius: 5px; cursor: pointer; margin-top: 15px;">
                    关闭
                </button>
            </div>
        `;

        searchOverlay.showCustomPanel('快捷键说明', shortcutsHtml, () => {
    ...
    updateDetectionStatus(status) {
        ...
        const initialHeightElement = document.getElementById('initial-height');
        const keyboardStateElement = document.getElementById('keyboard-state');
        
        if (!status) {
            status = mobileAdapter.getKeyboardStatus();
        }
        
        if (statusElement) {
            statusElement.textContent = `使用 ${window.visualViewport ? 'visualViewport' : '窗口大小'} 检测`;
        }
        if (currentHeightElement) {
            currentHeightElement.textContent = `${status.viewportHeight}px`;
        }
        if (initialHeightElement) {
            initialHeightElement.textContent = `${status.initialViewportHeight}px`;
        }
        if (keyboardStateElement) {
            keyboardStateElement.textContent = status.visible ? '显示' : '隐藏';
        }
    }
};
...
const appInitializer = {
    ...
    init() {
        try {
            ...
            mobileAdapter.init(); // 初始化移动端适配（增强版）

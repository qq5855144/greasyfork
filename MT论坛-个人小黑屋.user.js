...
(function () {
  ...
  function injectSidebarMenu() {
    ...
    if (!ul) return;
    var items = ul.querySelectorAll(':scope > LI.comiis_left_Touch');
    var anchor = null;
    for (var i = 0; i < items.length; i++) {
      var a = items[i].querySelector('a[href*="ac=promotion"]');
      if (a) { anchor = items[i]; break; }
    }
    if (!anchor) return;
    var li = document.createElement('li'); li.className = 'comiis_left_Touch';
    var link = document.createElement('a');
    link.href = 'javascript:;'; link.className = 'comiis_left_Touch';
    link.innerHTML = '<i class="comiis_font comiis_left_Touch" style="color:#3f8cff;">🕳️</i>个人小黑屋';
    li.appendChild(link);
    anchor.parentNode.insertBefore(li, anchor.nextSibling);
    li.addEventListener('click', function (e) {
      e.preventDefault();
      document.body.classList.remove('comiis_leftnv1');
      document.body.classList.add('comiis_leftnv0');
      openPanel();
    });
  }
  injectSidebarMenu();

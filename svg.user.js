...
(function() {
    ...
    class ImageCollectorService {
     ...
     destroy(){try{this.observer?.disconnect();this.po?.disconnect()}catch(_){}this.observer=null;this.po=null}
     static getFileExtension(u){try{return new URL(u,location.href).pathname.match(/\.([a-z0-9]{1,10})$/i)?.[1]||''}catch(_){return''}}
     static truncateTo4Bytes(s,max=CONFIG.image.infoTruncateLength){if(!s)return'';let r='',n=0;for(const ch of String(s)){const b=ch.codePointAt(0)<=0x7f?1:ch.codePointAt(0)<=0x7ff?2:3;if(n+b>max)return r+'...';r+=ch;n+=b}return r}
     static getImageName(u,alt){return alt||this._nameFallback(u)}
     static _nameFallback(u){try{return decodeURIComponent(new URL(u,location.href).pathname.split('/').filter(Boolean).pop()||'未命名图片').replace(/\.[^.]+$/,'')}catch(_){return'未命名图片'}}
    }
    ...
    class MediaCollectorService{
     ...
     _hookFetchFallback(){const f=window.fetch;if(typeof f!=='function'||f.__rsFallback)return;const self=this;const w=function(){let u='';try{const x=arguments[0];u=typeof x==='string'?x:x?.url||''}catch(_){}return f.apply(this,arguments).then(r=>{try{const full=self._norm(r.url||u),ct=r.headers.get('content-type')||'',t=self._mimeType(ct)||self._guessType(full);if(t){self._add(full,ct,t,'fetch-fallback');if(t==='video'&&self._isManifest(ct,full))self._parseManifestResponse(r.clone(),full)}}catch(_){}return r})};w.__rsFallback=true;try{window.fetch=w}catch(_) {}}
     _hookXHRFallback(){const o=XMLHttpRequest.prototype.open,s=XMLHttpRequest.prototype.send;if(o.__rsFallback)return;const self=this;XMLHttpRequest.prototype.open=function(m,u){this.__rsUrl=u;return o.apply(this,arguments)};XMLHttpRequest.prototype.send=function(){this.addEventListener('loadend',function(){try{const full=self._norm(this.responseURL||this.__rsUrl),ct=this.getResponseHeader('content-type')||'',t=self._mimeType(ct)||self._guessType(full);if(t)self._add(full,ct,t,'xhr-fallback')}catch(_){}},{once:true});return s.apply(this,arguments)};try{XMLHttpRequest.prototype.open.__rsFallback=true}catch(_) {}}

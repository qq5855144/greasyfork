...
const STORAGE_KEYS = Object.freeze({
    ...
    KEYBOARD_OFFSET: 'keyboardOffset' // 新增：键盘偏移量
});
...
const DEFAULT_CONFIG = {
    ...
    DRAG_SORT_DELAY: 500,
    KEYBOARD_OFFSET: 200 // 新增：默认键盘偏移量(px)
};
...
const appState = {
    ...
    keyboardOffset: GM_getValue(STORAGE_KEYS.KEYBOARD_OFFSET, DEFAULT_CONFIG.KEYBOARD_OFFSET), // 新增
    isKeyboardVisible: false // 新增：键盘显示状态
};
...
const keyboardDetector = {
    ...
    detectKeyboardState() {
        ...
        const checkKeyboardState = () => {
            ...
            // 如果窗口高度显著减少（通常键盘弹出时高度减少超过150px）
            if (newViewportHeight < initialViewportHeight - 150) {
                if (!appState.isKeyboardVisible) {
                    appState.isKeyboardVisible = true;
                    this.handleKeyboardShow();
                }
            } 
            // 如果窗口高度恢复到接近初始高度
            else if (newViewportHeight > initialViewportHeight - 50) {
                if (appState.isKeyboardVisible) {
        ...
        window.addEventListener('resize', () => {
            debounceUtils.debounce('keyboard_detect', checkKeyboardState, 100);
        });

        // 初始检测
        setTimeout(checkKeyboardState, 1000);
    },

    /**
     * 设置ResizeObserver监听可视区域变化
     */
    setupResizeObserver() {
        if (typeof ResizeObserver !== 'undefined') {
            ...
            if (visualViewport) {
                ...
                observer.observe(visualViewport);
            }
        }
    },
     ...
     */
    handleKeyboardShow() {
        if (appState.keyboardOffset > 0) {
            domHandler.applyKeyboardOffset(true);
        }
    },
     ...
     */
    handleKeyboardHide() {
        domHandler.applyKeyboardOffset(false);
    },

    /**
     * 更新键盘偏移量设置
     * @param {number} offset - 新的偏移量(px)
     */
    updateKeyboardOffset(offset) {
        appState.keyboardOffset = offset;
        GM_setValue(STORAGE_KEYS.KEYBOARD_OFFSET, offset);
        ...
        `;
                ...
                monitorInputFields() {
                    ...
                    /**
     * 应用键盘偏移
     * @param {boolean} apply - 是否应用偏移
     */
    applyKeyboardOffset(apply) {
        const punkJetBox = document.getElementById("punkjet-search-box");
        if (!punkJetBox) return;

        if (apply && appState.keyboardOffset > 0) {
            // 应用键盘偏移
            punkJetBox.style.transform = `translateY(-${appState.keyboardOffset}px)`;
            punkJetBox.style.transition = 'transform 0.3s ease';
        } else {
            // 恢复原位
    ...
    updateSearchBoxPosition() {
        ...
        // 如果键盘可见且设置了偏移量，应用偏移
        if (appState.isKeyboardVisible && appState.keyboardOffset > 0) {
            punkJetBox.style.transform = `translateY(-${appState.keyboardOffset}px)`;
        } else {
            // 显示/隐藏状态切换
        ...
        const menuItems = [
            ...
            },
            {
                icon: 'keyboard',
                text: '键盘偏移设置',
                action: () => this.showKeyboardOffsetSettings()
            },
            {
                icon: 'keyboard',
                text: '快捷键说明',
    ...
    showKeyboardOffsetSettings() {
        ...
        title.innerHTML = utils.createInlineSVG('keyboard') + ' 键盘偏移设置';
        title.style.cssText = `
            margin: 0 0 20px 0;
            color: #2c3e50;
            display: flex;
        ...
        description.textContent = "设置输入法键盘弹出时，搜索引擎栏向上移动的距离（像素）。设置为0可禁用此功能。";
        ...
        // 当前设置显示
        const currentSetting = document.createElement("div");
        currentSetting.style.cssText = `
            background: #f8f9fa;
            padding: 15px;
            border-radius: 8px;
            margin-bottom: 20px;
        ...
        currentSetting.innerHTML = `
            <div style="font-size: 12px; color: #7f8c8d; margin-bottom: 5px;">当前偏移量</div>
            <div style="font-size: 24px; font-weight: bold; color: #2c3e50;">${appState.keyboardOffset}px</div>
        `;
        ...
        sliderLabel.textContent = `偏移量: ${appState.keyboardOffset}px`;
        ...
        slider.max = "500";
        slider.step = "10";
        slider.value = appState.keyboardOffset;
        ...
        const presets = [
            { value: 0, label: '禁用', desc: '不移动' },
            { value: 150, label: '小', desc: '150px' },
            { value: 250, label: '中', desc: '250px' },
            { value: 350, label: '大', desc: '350px' },
            { value: 200, label: '推荐', desc: '200px' },
            { value: 100, label: '微小', desc: '100px' }
        ];

        presets.forEach(preset => {
            const presetBtn = document.createElement("button");
            presetBtn.textContent = preset.label;
            presetBtn.title = preset.desc;
            presetBtn.style.cssText = `
                padding: 10px 5px;
                border: 1px solid #ddd;
                background: ${preset.value === appState.keyboardOffset ? '#3498db' : 'white'};
                color: ${preset.value === appState.keyboardOffset ? 'white' : '#333'};
                border-radius: 5px;
                cursor: pointer;
                font-size: 12px;
            ...
            presetBtn.addEventListener('click', () => {
                ...
                sliderLabel.textContent = `偏移量: ${preset.value}px`;
                currentSetting.innerHTML = `
                    <div style="font-size: 12px; color: #7f8c8d; margin-bottom: 5px;">当前偏移量</div>
                    <div style="font-size: 24px; font-weight: bold; color: #2c3e50;">${preset.value}px</div>
                `;
                ...
                presetBtn.style.background = '#3498db';
        ...
        slider.addEventListener('input', () => {
            ...
            sliderLabel.textContent = `偏移量: ${value}px`;
            currentSetting.innerHTML = `
                <div style="font-size: 12px; color: #7f8c8d; margin-bottom: 5px;">当前偏移量</div>
                <div style="font-size: 24px; font-weight: bold; color: #2c3e50;">${value}px</div>
            `;
        ...
        saveBtn.addEventListener('click', () => {
            const newOffset = parseInt(slider.value);
            keyboardDetector.updateKeyboardOffset(newOffset);
            modal.remove();
            sliderStyle.remove();
            
            // 显示成功提示
            const successMsg = document.createElement("div");
            successMsg.textContent = `✅ 键盘偏移量已设置为 ${newOffset}px`;
        ...
        content.appendChild(currentSetting);
        ...
        // 应用焦点陷阱
        accessibility.trapFocus(content);
    },
